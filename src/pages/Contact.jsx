...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    ...
    emailjs.send(import.meta.env.VITE_APP_EMAILJS_SERVICE_ID, import.meta.env.VITE_APP_EMAILJS_TEMPLATE_ID, {
      ...
      to_email: 'charlesmcquain@gmail.com',
    ...
    <section className="relative flex lg:flex-row flex-col max-container">
      ...
      <div className="flex-1 min-w-[50%] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-7 mt-14">
          ...
          <textarea name="message" className="textarea" placeholder="Let me know how I can help you!" required value={form.message} 
           onChange={handleChange} onFocus={handleFocus} onBlur={handleBlur}/>
           <button type="submit" disabled={isLoading} className="btn" onFocus={handleFocus} onBlur={handleBlur}>{isLoading ? 'Sending...' : 'Send Message'}</button>
        </form>
      </div>
      <div className="lg:w-1/2 w-full lg:h-auto md:h-[550px] h-[350px]">
        <Canvas camera={{position: [0, 0, 5], fox: 75, near: 0.1, far: 1000}}>
          ...
          <Suspense fallback={<Loader />}>
            <Fox position={[0.5, 0.35, 0]} rotation={[12.629, -0.6, 0]} scale={[0.5, 0.5, 0.5]} currentAnimation={currentAnimation} />
          </Suspense>
        </Canvas>
      </div>
    </section>
